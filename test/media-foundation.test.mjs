...
test('timeline save, conflict, undo and redo use the existing record',()=>{const p=createEdit(),e=timeline(p),r=studio.saveTimeline(p.id,1,e);assert.equal(r.plan.version,4);assert.equal(r.revision,2);assert.throws(()=>studio.saveTimeline(p.id,1,e),/revision_conflict/);const u=studio.undoTimeline(p.id,2);assert.equal(u.timeline.clips.length,0);const back=studio.redoTimeline(p.id,3);assert.equal(back.plan.frames,60);});
