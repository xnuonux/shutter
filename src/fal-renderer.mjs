...
export class FalRenderer {
 ...
 budget() {
  ...
  for(const j of this.studio.listJobs().filter(isH3))if(j.charge){const p=result[j.charge.pool];if(j.charge.actualUsd!=null)p.spent+=j.charge.actualUsd;else p.reserved+=j.charge.reservedUsd;}
 ...
 async prepare(id) {
  ...
  const promo=Date.now()<Date.parse('2026-09-14T00:00:00Z');
  const baseRate=(ref||!promo) ? .05 : .0125;
  const outputRate=baseRate*(g.resolution==='768P'?1.6:1);
  const estimatedUsd=rounded(outputRate*g.duration+(ref?Math.max(0,tokens-4096)*.02/1000:0));
  const quote={pool:g.mode==='text-to-video'?'text':'imageReference',estimatedUsd,reservedUsd:Math.ceil(estimatedUsd*1000)/1000,baseRate,outputRate,referenceTokens:tokens,measured,preparedAt:new Date().toISOString(),expiresAt:new Date(Date.now()+3600000).toISOString()};
 ...
 async submit(id) {
  ...
  const current=this.studio.getProduction(job.projectId);
  if(current.revision!==job.snapshot.projectRevision)throw Error('prepared_revision_conflict');
  for(const a of job.snapshot.references)this.studio.verifyAsset(a.id);
  const {data}=await this.request(this.pricingBase+'?endpoint_id='+encodeURIComponent(job.snapshot.workflow));
  const pricing=data.prices?.find(p=>p.endpoint_id===job.snapshot.workflow);
  if(!pricing||pricing.currency!=='USD'||pricing.unit!=='seconds'||pricing.unit_price!==job.quote.baseRate)throw Error('fal_pricing_changed');
 ...
 async reconcile(id) {
  ...
  try {
   ...
   // The first real five-second Max result contains 124 frames at 24 fps.
   // Retain this short padding and its measured timing; never relabel it as exactly five seconds.
