...
export class FalRenderer {
 ...
 async prepare(id) {
  ...
  let tokens=0;
  if(ref) {
   for(const mediaKind of ['video','audio']){const clips=measured.filter(m=>m.kind===mediaKind);if(clips.some(m=>!Number.isFinite(m.duration)||m.duration<2||m.duration>15)||clips.reduce((s,m)=>s+m.duration,0)>15.001)throw Error('h3_reference_duration');}
  ...
  const outputUsd=rounded(outputRate*g.duration),referenceInputUsd=rounded(ref?Math.max(0,tokens-4096)*.02/1000:0),estimatedUsd=rounded(outputUsd+referenceInputUsd);
  const quote={pool:g.mode==='text-to-video'?'text':'imageReference',estimatedUsd,reservedUsd:Math.ceil(estimatedUsd*1000)/1000,baseRate,outputRate,outputUsd,referenceInputUsd,referenceTokens:tokens,referenceTokenRate:ref?.02:0,
    variableReferenceCost:ref&&measured.some(m=>['video','audio'].includes(m.kind)),referenceCostBasis:ref?'conservative token estimate; provider billable units settle actual cost after completion':'none',
    measured,pricing:{endpointId:pricing.endpoint_id,currency:pricing.currency,unit:pricing.unit,unitPrice:pricing.unit_price},validatedLiveOn:'2026-09-12',
    preparedAt:new Date().toISOString(),expiresAt:new Date(Date.now()+3600000).toISOString()};
 ...
 async submit(id) {
  ...
  if(Number.isSafeInteger(job.snapshot.timelineRevision)){if(this.studio.getTimeline(job.projectId).revision!==job.snapshot.timelineRevision)throw Error('prepared_revision_conflict');}else{const current=this.studio.getProduction(job.projectId);if(current.revision!==job.snapshot.projectRevision)throw Error('prepared_revision_conflict');}
  ...
  const pricing=await this.pricing(job.snapshot.workflow);if(pricing.unit_price!==job.quote.baseRate)throw Error('fal_pricing_changed');
  ...
  try {const {data:receipt}=await this.request(this.queueBase+'/'+job.snapshot.workflow,{method:'POST',body:JSON.stringify(input)});if(!receipt.request_id||!receipt.status_url||!receipt.response_url)throw Error('missing_provider_receipt');return this.studio.updateJob(id,{state:'rendering',providerId:receipt.request_id,receipt,error:null});}catch(e){this.studio.updateJob(id,{state:'unknown',error:e.message});throw e;}
 }
 async reconcile(id) {
  ...
  try {
   ...
   const result=await this.request(job.receipt.response_url),units=result.units===null?null:Number(result.units),charge={...job.charge,...(Number.isFinite(units)&&units>=0?{billableUnits:units,actualUsd:rounded(units*job.pricing.unit_price)}:{})};
   const billing=charge.actualUsd==null?{status:'unreconciled'}:{status:'reconciled',estimatedUsd:job.quote?.estimatedUsd??null,reservedUsd:job.quote?.reservedUsd??null,actualUsd:charge.actualUsd,deltaFromEstimateUsd:job.quote?.estimatedUsd==null?null:rounded(charge.actualUsd-job.quote.estimatedUsd)};
   this.studio.updateJob(id,{state:'verifying',result:result.data,charge,billing});
   const url=new URL(result.data.video?.url);if(!((url.protocol==='https:'&&(url.hostname==='fal.media'||url.hostname.endsWith('.fal.media')))||url.origin===new URL(this.queueBase).origin))throw Error('unexpected_media_host');
   ...
   const media=await this.probe(staging),g=job.snapshot.shot.generation;if(media.kind!=='video'||!media.decoded||!Number.isFinite(media.duration)||Math.abs(media.duration-g.duration)>.25||Math.abs(media.fps-24)>.01||Math.abs(Math.min(media.width,media.height)-Number(g.resolution.slice(0,-1)))>16)throw Error('video_profile_mismatch');
   const asset=this.studio.importAsset(bytes,{name:job.snapshot.shot.title+'.mp4',origin:'fal H3 Max; job '+id+'; provider '+job.providerId});fs.unlinkSync(staging);staging=null;
   return this.studio.updateJob(id,{state:'ready',output:asset.id,media,billing,timingDifferenceSeconds:rounded(media.duration-g.duration),finishedAt:new Date().toISOString(),error:charge.actualUsd==null?'billing_reconciliation_required':null});
