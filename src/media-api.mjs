...
export async function handleMediaRequest(studio,req,res,url) {
  ...
  try {
    if(req.method==='GET'&&url.pathname==='/api/media/state') {
      ...
      json(res,200,{productions:studio.list('production'),timelines,derivations:studio.list('media-derivation'),assets:studio.list('asset').map(a=>({...a,media:profiles.get(a.id)||null})),cuts:studio.list('cut').filter(c=>c.plan?.format===MEDIA_EDIT_FORMAT)});return true;
    }
  ...
  }catch(e){if(!res.headersSent)json(res,/conflict|busy/.test(e.message)?409:/unavailable/.test(e.message)?503:e.message==='not_found'?404:e.message==='asset_size'?413:400,{error:e.message});else res.destroy();return true;}
}
