...
export function h3Snapshot(studio,p,shot) {
 ...
 if(g.mode==='image-to-video') {
  ...
  add(shot.reference,'authoritative current visible appearance and intended shot state; appearance wins if another reference conflicts');
...
export function h3Input(job,urlFor) {
 ...
 const prompt=[s.style,g.mode==='reference-to-video'?refs.map(r=>r.label+' controls '+r.role+'.').join('\n'):'',
  ...
  s.place.name?'Location: '+s.place.name:'',s.shot.before?'Opening state: '+s.shot.before:'',s.shot.action,s.shot.camera?'Camera: '+s.shot.camera:'',s.shot.after?'Visible ending state: '+s.shot.after:'',s.shot.sound?'Sound and dialogue: '+s.shot.sound:'Diegetic scene audio only. No non-diegetic music, score, or captions.'].filter(Boolean).join('\n');
