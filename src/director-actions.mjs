...
export function actionCatalog(types=[]){
  ...
  return {version:ACTION_VERSION,scope:'asset-backed-studio',limits:{commandsPerBatch:32},
    ...
    workflow:['Read action-context and retain the observed revision.','Request the needed action schemas by type.','For a chosen shot proposal, preserve proposal={proposalId,momentId} and exactly its single candidate command through preview, evidence and apply.','Preview a complete batch. Inspect changed fields, visible source intervals and warnings.','Apply with the exact commands, optional proposal, baseRevision, previewHash and a stable requestKey.','On an uncertain response, read the receipt or retry identical input with the same key. Never invent a result.','Review the saved result with shutter_review_scene at its current revision; compare actual picture and available sound with saved intent. A URL is not perception, and sampled images leave gaps. Revise or undo a mismatch through a fresh preview/apply cycle.'],
    ...
    relatedTools:['shutter_studio_context','shutter_inspect_source','shutter_search_moments','shutter_save_moment','shutter_get_direction','shutter_save_direction','shutter_propose_shots','shutter_inspect_cutaway','shutter_review_scene'],
