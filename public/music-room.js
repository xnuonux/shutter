...
export class MusicRoom {
  constructor({root,program,getEdit,getState,getSource,onEdit,notify,api,action,isBusy}) {
    ...
    this.onKey=e=>{
      if(!this.getEdit()||this.isBusy()||e.altKey||e.ctrlKey||e.metaKey||e.target.closest('input,textarea,select,[contenteditable="true"]')||document.querySelector('dialog[open]'))return;
  ...
  draw(){
    ...
    ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);ctx.fillStyle='#14151b';ctx.fillRect(0,0,w,h);ctx.font='11px system-ui';
