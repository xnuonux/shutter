...
export class DirectionRoom {
  constructor(options){
    ...
    const change=()=>{
      this.pending=JSON.stringify(this.form())!==JSON.stringify(this.direction?.brief||empty());this.clearAudition();this.renderReview();this.refreshStatus();
    };
  ...
  async accept(){
    ...
    this.clearAudition();this.candidate=null;this.applySaved(record);this.renderReview();this.refreshStatus();this.notify(coverage?'Cutaway accepted. The main footage keeps its clock. Undo restores the previous cut.':'Shot accepted. Timing and coverage are preserved. Undo restores the previous cut.');
  }
}
